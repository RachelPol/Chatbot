...
namespace ListPlugin
{
    ...
    public class ListPlugin : IPlugin
    {
        ...
        public PluginOutput Execute(PluginInput input)
        {
            ...
            {
                string wordToDelete = input.Message.Substring("delete ".Length).Trim();
                int index = list.IndexOf(wordToDelete);

                if (index != -1)
                {
                    list.RemoveAt(index);
                    var data = new PersistentDataStructure(list);
                    return new PluginOutput($"Deleted word: {wordToDelete}", JsonSerializer.Serialize(data));
                }
                else
                {
                    return new PluginOutput($"Word '{wordToDelete}' not found in the list.");
                }
            }
            else if (input.Message.StartsWith("delete", StringComparison.OrdinalIgnoreCase))
            {
